...
const TOTAL_SKILLS = Object.keys(SkillType).length;
const SKILL_ORDER = Object.keys(SkillType) as SkillType[];
...
export async function getStudentOverview(studentId: string): Promise<StudentOverview> {
  const [testsCompleted, resultsBySkill, attemptedSkills, pendingWriting, pendingSpeaking] =
    await Promise.all([
      prisma.result.count({ where: { studentId, completedAt: { not: null } } }),
      prisma.result.groupBy({
        by: ["skill"],
        where: { studentId, completedAt: { not: null }, bandScore: { not: null } },
        _avg: { bandScore: true },
      }),
      ...
      prisma.speakingSubmission.count({ where: { studentId, status: { not: "REVIEWED" } } }),
    ]);
  ...
  return {
    ...
    progressPercent,
    weakestSkill,
    pendingReviews: pendingWriting + pendingSpeaking,
  };
}
...
/** A unified, real activity feed merged from completed Results and Writing/Speaking submissions. */
export async function getRecentActivity(studentId: string, limit = 5): Promise<ActivityItem[]> {
  ...
  const items: ActivityItem[] = [
    ...
    ...speaking.map((submission) => ({
      ...
      title: `Speaking Part ${submission.part}`,
      detail:
        submission.status === "REVIEWED"
          ? `Band ${submission.bandScore?.toFixed(1) ?? "—"}`
          : "Awaiting review",
